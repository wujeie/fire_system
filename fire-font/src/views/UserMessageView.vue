...
const getMessages = async () => {
  const res = await request.get('http://localhost:8080/message/selectByUserId',{
    params:{
...
const submitReplyForm = async ()=>{
  const res = await request.post('http://localhost:8080/message/addMessage',messageReplyForm)
  if(res.data.code === 1){
...
<template>
  <el-container style="min-height: 100vh; min-width: 100vw; overflow: hidden;">
    ...
    <el-container style="height: 100vh; width: 100%; display: flex; flex-direction: column;">
      ...
      <el-main>
        <el-button @click="addMessage">立即留言</el-button>
        <el-table :data="messages" style="width: 100%">
          <el-table-column type="index" :index="indexMethod" label="序号" width="200" />
          <el-table-column prop="content" label="我的留言" width="300">
          ...
          <el-table-column label="操作">
            <template #default="scope">
              <span><el-button @click="ShowMessage(scope.row.feedbackId)">查看详情</el-button></span>
            </template>
          </el-table-column>
        </el-table>

        <!--  留言回复表单-->
        <el-dialog :append-to-body="true" v-model="dialogReplyVisible" title="留言内容" width="50%">
          <el-form :model="messageReplyForm" ref="detailFormRef" label-width="120px">
            <el-form-item label="留言内容">
              <el-input type="textarea" v-model="messageReplyForm.content" :rows="15"></el-input>
            </el-form-item>
          <el-form-item>
